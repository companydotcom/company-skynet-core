...
const handleBulk = async (request: middy.Request, options: SettledOptions) => {
  ...
  if (options.isBulk) {
    ...
    const middyInternal: any = await getMiddyInternal(request, ['AWS']);
    ...
    request.event = messagesToProcess.map((m: eventTypes.SNSEvent) =>
      sqsParser(m),
    );
  }
};
...
export const getSSMParams = async ({
  ...
  const ssmClient = new AWS.ssmClient.SSMClient({ region });
  const params = {
    Names: keys,
    WithDecryption: true,
  };
  try {
    const command = new AWS.ssmClient.GetParametersCommand(params);
      ...
      await Promise.all(
        handledMessages.map(async (message: HandledSkynetMessage) => {
          ...
          if (workerResp.status && workerResp.status === 'fail') {
            sendToDlq(middeyInternal.AWS, message, options, workerResp.error);
          }
          ...
          if (
            ...
            Buffer.byteLength(JSON.stringify(workerResp.res), 'utf-8') > 75000
          ) {
    ...
    if (request.response && request.response.length) {
      ...
      await Promise.all(
        handledMessages.map(async (m) => {
          await sendToDlq(middeyInternal.AWS, m, options, request.error);
        }),
      );
    }
  };
